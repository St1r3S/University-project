...
import ua.com.foxminded.university.dao.jdbc.mappers.AcademicYearRowMapper;
...
@Repository
public class JdbcAcademicYearDao extends AbstractCrudDao<AcademicYear, Long> implements AcademicYearDao {
    ...
    public JdbcAcademicYearDao(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
        this.jdbcInsert = new SimpleJdbcInsert(jdbcTemplate).withTableName("academic_years").usingGeneratedKeyColumns("id");
    }
    ...
    protected AcademicYear create(AcademicYear entity) {
        Number id = jdbcInsert.executeAndReturnKey(
                new MapSqlParameterSource()
                        .addValue(ACADEMIC_YEAR_YEAR_NUMBER, entity.getYearNumber())
                        .addValue(ACADEMIC_YEAR_SEMESTER_TYPE, entity.getSemesterType().getValue())
        );
    ...
    protected AcademicYear update(AcademicYear entity) {
        if (1 == jdbcTemplate.update(UPDATE, entity.getYearNumber(), entity.getSemesterType().getValue(), entity.getId())) {
            return entity;
        }

...
@Repository
public class JdbcScheduleDayDao extends AbstractCrudDao<ScheduleDay, Long> implements ScheduleDayDao {
    ...
    public JdbcScheduleDayDao(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
        this.jdbcInsert = new SimpleJdbcInsert(jdbcTemplate).withTableName("schedule_days").usingGeneratedKeyColumns("id");
    }
    ...
    protected ScheduleDay create(ScheduleDay entity) {
        Number id = jdbcInsert.executeAndReturnKey(
                new MapSqlParameterSource()
                        .addValue(SCHEDULE_DAY_DAY_OF_WEEK, entity.getDayOfWeek().getValue())
                        .addValue(SCHEDULE_DAY_SEMESTER_TYPE, entity.getSemesterType().getValue())
        );
    ...
    protected ScheduleDay update(ScheduleDay entity) {
        if (1 == jdbcTemplate.update(UPDATE, entity.getDayOfWeek().getValue(), entity.getSemesterType().getValue(), entity.getId())) {
            return entity;
        }
